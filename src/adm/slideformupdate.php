...
if($w == "")
{
    $sql = " insert {$g5['slide_table']} set $sql_common ";
    sql_query($sql);
...
{
    $sql = " update {$g5['slide_table']} set $sql_common where nw_id = '$nw_id' ";
    sql_query($sql);
}
else if ($w == "d")
{
    $sql = " delete from {$g5['slide_table']} where nw_id = '$nw_id' ";
    sql_query($sql);
}

if ($w == "d")
{
    goto_url('./sldielist.php');
}
else
{
    goto_url("./slideform.php?w=u&amp;nw_id=$nw_id");
}
?>
