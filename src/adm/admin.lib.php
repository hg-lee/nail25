...
// 접근 권한 검사
$tmp1=explode('/',$_SERVER['REQUEST_URI']);
$adm_fold=$tmp1[1];
error_log($_SERVER['REQUEST_URI']."|".G5_URL."|".$member['mb_id']);
if (!$member['mb_id'])
{
    //if($adm_fold=='Partner_Center' || G5_URL=="http://pc.srman.net") alert('로그인 하십시오.', G5_BBS_URL.'/login.php?url=' . urlencode(G5_ADMIN_PC_R));
    error_log("[SRman] ".$member['mb_id']);
    //if($adm_fold=='Partner_Center' || G5_URL=="http://pc.srman.net") echo "<script>location.href=".G5_BBS_URL."/login.php?url=".urlencode(G5_URL).";</script>";
    if($adm_fold=='Partner_Center' || G5_URL=="http://partner.srman.net"){
        //alert('로그인 하십시오.', G5_BBS_URL.'/login.php?url=' . urlencode(G5_URL));

        echo "<script>
            location.href='http://partner.srman.net/bbs/login.php';
        </script>";

    }else{
        alert('로그인 하십시오.', G5_BBS_URL.'/login.php?url=' . urlencode(G5_ADMIN_DIR));
    }
}
else if ($is_admin != 'super')
{
    ...
    if (!$i)
    {
        if($adm_fold=='Partner_Center'|| G5_URL=="http://partner.srman.net") error_log(G5_URL);
        else alert('최고관리자 또는 관리권한이 있는 회원만 접근 가능합니다.)', G5_URL);
    }
}
...
while ($entry = $tmp->read()) {
    if($member['mb_level']==10){
        ...
        if (!preg_match('/^shopManager.menu([0-9]{4}).*\.php$/', $entry, $m))
        continue;  // 파일명이 menu 으로 시작하지 않으면 무시한다.
    }
