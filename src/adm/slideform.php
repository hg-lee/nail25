...
{
    $html_title .= " 수정";
    $sql = " select * from {$g5['slide_table']} where nw_id = '$nw_id' ";
    $nw = sql_fetch($sql);
...
<form name="frmnewwin" action="./slideformupdate.php" onsubmit="return frmnewwin_check(this);" method="post">
...
<div class="tbl_frm01 tbl_wrap">
    ...
    <tr style="display:none">
        ...
        <td>
           <input type="text" name="nw_left" value="0" id="nw_left" required class="frm_input required" size="5"> px
        </td>
    </tr>
    <tr style="display:none">
        <th scope="row"><label for="nw_top">팝업레이어 상단 위치<strong class="sound_only"> 필수</strong></label></th>
        <td>
            <input type="text" name="nw_top" value="0" id="nw_top" required class="frm_input required"  size="5"> px
        </td>
    </tr>
    <tr style="display:none">
    ...
    <tr>
        <th scope="row"><label for="nw_content">주의사항</label></th>
        <td><u><strong>이미지의 사이즈는 가로 <?=$nw['nw_width'];?> 픽셀 x 세로 <?=$nw['nw_height'];?>  픽셀으로 올려주세요. </strong></u></td>
    </tr>
