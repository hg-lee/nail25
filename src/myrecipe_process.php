...
*/
    ...
    if($mode=='m' || $mr_id>0){
        $sql = " UPDATE Nail25_MyRecipe SET mr_subject='', mr_contents='', mr_hashtag='$hashtags', mr_comp_img='$comp_img_dir' ";
    ...
    if($rst){
        if($is_comp=='N'){
            ...
            <script>
                ...
                //history.back();
                //location.reload();
                location.href='myrecipe_add.php?mode=m&mr_id=<?=$last_id?>';
                //opener.location.reload();
            </script>
            <?
        }else alert('정상처리 되었습니다.',G5_DOMAIN);
    }else{
