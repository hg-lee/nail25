...
function sql_field_count($table, $link=null){
    global $g5;
    ...
    if(function_exists('mysqli_num_fields') && G5_MYSQLI_USE) {
...
function get_tag_list($link=null){
    $cnt = 0;
    $no=1;
    $sql = " SELECT * FROM Nail25_hashtag ORDER BY type, idx ";
    $result = sql_query($sql, $link);
    while ($row = sql_fetch_array($result))
    {
        $n_no=$row['type'];
