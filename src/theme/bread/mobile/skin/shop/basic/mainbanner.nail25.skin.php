    ...
    if (file_exists($bimg))
    {
       ...
       echo '<li>'.PHP_EOL;
        if ($row['bn_url'][0] == '#')
            ...
            //echo $banner.'<span  style="background-image:url('.G5_DATA_URL.'/banner/'.$row['bn_id'].') no-repeat center;" class="bn-img"><span class="bn-txt-wr"><span class="bn-txt">'.$row['bn_alt'].'<br><span class="btn_b02">자세히보기</span></span></span></span>';
            echo $banner.'<img src="'.G5_DATA_URL.'/banner/'.$row['bn_id'].'" width="'.$size[0].'" alt="'.get_text($row['bn_alt']).'"'.$bn_border.'>';
            if($banner)
                echo '</a>'.PHP_EOL;
        }
        else {
            //echo $banner.'<span  style="background-image:url('.G5_DATA_URL.'/banner/'.$row['bn_id'].') no-repeat center;" class="bn-img"><span class="bn-txt-wr"><span class="bn-txt">'.$row['bn_alt'].'</span></span></span>';
            echo $banner.'<img src="'.G5_DATA_URL.'/banner/'.$row['bn_id'].'" width="'.$size[0].'" alt="'.get_text($row['bn_alt']).'"'.$bn_border.'>';
        }
        echo '</li>'.PHP_EOL;
